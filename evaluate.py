...
# save a 2x4 grid of stacked area charts showing action proportions during training
def plot_action_distribution(training_results, group_keys, group_labels):
    ...
    fig.suptitle('action distribution during training, rolling window 200',
                 fontsize=14, fontweight='bold')
    fig.tight_layout(rect=[0, 0.05, 1, 0.95])
    fig.savefig('plots/action_distribution.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("saved plots/action_distribution.png")


# save a 2x4 grid of bar charts showing q values for a fixed board state across all groups
def plot_qvalue_heatmap(group_keys, group_labels):
    ...
    fig.suptitle('Q-Value Comparison on Fixed Board State',
                 fontsize=14, fontweight='bold')
    fig.tight_layout(rect=[0, 0.08, 1, 0.95])
    fig.savefig('plots/qvalue_heatmap.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("saved plots/qvalue_heatmap.png")


# generate animated gif replays for all trained groups and the random baseline
def generate_gameplay_gifs():
    ...
    # generate a gif for each trained group and the random baseline
    configs = []
    for gid in range(1, 9):
        configs.append((gid, gid > 4, f'plots/replay_group_{gid}.gif'))
    configs.append((None, False, 'plots/replay_random.gif'))
...
# evaluate parameter study groups and plot score vs penalty magnitude
def plot_parameter_study(num_eval_episodes=100, eval_results=None):
    # map each parameter study group to its checkpoint path
    ps_checkpoints = {
        'ps1': 'checkpoints/group_ps1.pth',
        'ps2': 'checkpoints/group_2.pth',
        'ps3': 'checkpoints/group_ps3.pth',
        'ps4': 'checkpoints/group_ps4.pth',
        'ps5': 'checkpoints/group_3.pth',
        'ps6': 'checkpoints/group_ps6.pth',
    }

    # reuse already computed group 1 scores as the no penalty reference if available
    if eval_results is not None and 'group_1' in eval_results:
        g1_avg = np.mean(eval_results['group_1']['scores'])
    else:
        agent = DQNAgent()
        agent.load('checkpoints/group_1.pth')
        g1_scores, _ = evaluate_agent(agent, num_eval_episodes)
        g1_avg = np.mean(g1_scores)

    # evaluate game over penalty groups at magnitudes 50, 100, 200, reusing cached scores where possible
    go_magnitudes = [50, 100, 200]
    go_avg_scores = []
    for ps_id, cache_key in [('ps1', None), ('ps2', 'group_2'), ('ps3', None)]:
        if cache_key is not None and eval_results is not None and cache_key in eval_results:
            go_avg_scores.append(np.mean(eval_results[cache_key]['scores']))
        else:
            agent = DQNAgent()
            agent.load(ps_checkpoints[ps_id])
            scores, _ = evaluate_agent(agent, num_eval_episodes)
            go_avg_scores.append(np.mean(scores))

    # evaluate invalid move penalty groups at magnitudes 2, 5, 10, reusing cached scores where possible
    inv_magnitudes = [2, 5, 10]
    inv_avg_scores = []
    for ps_id, cache_key in [('ps4', None), ('ps5', 'group_3'), ('ps6', None)]:
        if cache_key is not None and eval_results is not None and cache_key in eval_results:
            inv_avg_scores.append(np.mean(eval_results[cache_key]['scores']))
        else:
            agent = DQNAgent()
            agent.load(ps_checkpoints[ps_id])
            scores, _ = evaluate_agent(agent, num_eval_episodes)
            inv_avg_scores.append(np.mean(scores))

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle('Effect of Penalty Magnitude on Agent Performance',
                 fontsize=14, fontweight='bold')

    # left subplot for game over penalty
    ax1.plot(go_magnitudes, go_avg_scores, 'o-', color='steelblue', linewidth=2, markersize=8)
    ax1.axhline(y=g1_avg, color='gray', linestyle='--', label='no penalty', alpha=0.7)
    ...
    # right subplot for invalid move penalty
    ax2.plot(inv_magnitudes, inv_avg_scores, 'o-', color='crimson', linewidth=2, markersize=8)
    ax2.axhline(y=g1_avg, color='gray', linestyle='--', label='no penalty', alpha=0.7)
    ...
    fig.tight_layout(pad=2.0)
    fig.savefig('plots/parameter_study.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("saved plots/parameter_study.png")
...
def run_evaluation(num_eval_episodes=100):
    ...
    keys = [f'group_{i}' for i in range(1, 9)] + ['random']
    labels = [
        'G1 DQN, score only',
        'G2 DQN, game over penalty',
        'G3 DQN, invalid move penalty',
        'G4 DQN, both penalties',
        'G5 DDQN, score only',
    ...
    # action distribution for trained groups only
    trained_keys = [f'group_{i}' for i in range(1, 9)]
    trained_labels = labels[:8]
    plot_action_distribution(training_results, trained_keys, trained_labels)

    # q value comparison on a fixed board state
    plot_qvalue_heatmap(trained_keys, trained_labels)

    # animated gameplay replays
    generate_gameplay_gifs()

    # penalty magnitude parameter study
    plot_parameter_study(num_eval_episodes, eval_results)
