...
# train a single dqn agent for num_episodes with the given penalty configuration
...
# train the no-penalty baseline and all six parameter study groups fresh using standard dqn
def train_part_c(num_episodes=10000):
    # ps_baseline is the no-penalty reference; ps1-ps6 vary one penalty at a time
    group_configs = {
        'ps_baseline': (0, 0),
        'ps1': (-50, 0),
        'ps2': (-100, 0),
    ...
    for group_id, (go_penalty, inv_penalty) in group_configs.items():
        print(f"\ntraining {group_id} (dqn), "
              f"game over penalty {go_penalty}, "
              f"invalid move penalty {inv_penalty}")
        results[group_id] = train_group(
            group_id, go_penalty, inv_penalty, num_episodes, use_double_dqn=False
        )
    ...
    print("\npart c training complete, results saved to results/part_c_training_results.pkl")
    return results


if __name__ == '__main__':
    num_episodes = int(sys.argv[1]) if len(sys.argv) > 1 else 10000
    train_part_c(num_episodes)
