...
def plot_avg_score(eval_results, keys, labels, colors, out_path):
    ...
    ax.set_title('average game score per experiment group (part a)', fontsize=14,
...
def plot_avg_max_tile(eval_results, keys, labels, colors, out_path):
    ...
    ax.set_title('average max tile reached per experiment group (part a)', fontsize=14,
                 fontweight='bold', pad=12)
    ax.set_ylim(0, top * 1.18)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
...
def plot_reward_curves(training_results, keys, labels, colors, out_path):
    ...
    ax.set_title('training reward curves (part a)', fontsize=14,
                 fontweight='bold', pad=12)
...
def plot_action_distribution(training_results, group_keys, group_labels, out_path):
    ...
    fig.suptitle('action distribution during training, rolling window 200 (part a)',
...
# save a 1x4 grid of bar charts showing q values for a fixed board state across all groups
def plot_qvalue_heatmap(group_keys, group_labels, out_path):
    ...
    fig.suptitle('Q-Value Comparison on Fixed Board State (Part A)',
                 fontsize=14, fontweight='bold')
    fig.tight_layout(rect=[0, 0.08, 1, 0.95])
    fig.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
# generate animated gif replays for groups 1 through 4
def generate_gameplay_gifs():
    ...
    for gid in range(1, 5):
        save_path = f'plots/part_a_replay_group_{gid}.gif'
        env = Game2048()
...
# load checkpoints, evaluate groups 1 through 4, and generate all part a plots
def run_evaluation_a(num_eval_episodes=100):
    with open('results/part_a_training_results.pkl', 'rb') as f:
    ...
    plot_avg_score(eval_results, keys, labels, colors, 'plots/part_a_avg_score.png')
    plot_avg_max_tile(eval_results, keys, labels, colors, 'plots/part_a_avg_max_tile.png')
    plot_reward_curves(training_results, keys, labels, colors, 'plots/part_a_reward_curves.png')
    plot_action_distribution(training_results, keys, labels, 'plots/part_a_action_distribution.png')
    plot_qvalue_heatmap(keys, labels, 'plots/part_a_qvalue_heatmap.png')
    generate_gameplay_gifs()

    with open('results/part_a_eval_results.pkl', 'wb') as f:
        pickle.dump(eval_results, f)

    print("\npart a evaluation complete, plots saved to plots/")
    return eval_results
